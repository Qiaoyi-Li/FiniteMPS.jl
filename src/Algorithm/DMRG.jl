...
function DMRGSweep2!(Env::SparseEnvironment{L,3,T}; kwargs...) where {L,T<:Tuple{AdjointMPS,SparseMPO,MPS}}
     ...
     Al::MPSTensor = Ψ[1]
     for si = 1:L-1
          @timeit TO "pushEnv" canonicalize!(Env, si, si + 1)
          Ar = Ψ[si+1]

          @timeit TO "DMRGUpdate2" eg, xg, info_Lanczos = _DMRGUpdate2(ProjHam(Env, si, si + 1), Al, Ar; kwargs...)
          @timeit TO "svd" Ψ[si], s, vd, ϵ = tsvd(xg; trunc=trunc)
          # next Al
          Al = s * vd
     ...
     for si = reverse(2:L-1)
          @timeit TO "pushEnv" canonicalize!(Env, si - 1, si)
          Al = Ψ[si-1]

          @timeit TO "DMRGUpdate2" eg, xg, info_Lanczos = _DMRGUpdate2(ProjHam(Env, si - 1, si), Al, Ar; kwargs...)
          @timeit TO "svd" u, s, Ψ[si], ϵ = tsvd(xg; trunc=trunc)
          # next Ar
          Ar = u * s
          # remember to change Center of Ψ manually
          Center(Ψ)[:] = [si - 1, si - 1]
...
function _DMRGUpdate2(H::SparseProjectiveHamiltonian{2}, Al::MPSTensor, Ar::MPSTensor; kwargs...)
     ...
     prefuse = get(kwargs, :prefuse, false)

     if prefuse
          # El, Er, isoL, isoR = _prefuse2(H)
          # @tensor x0[a;g] := isoL'[a,b,c]*Al[b,c,d]*Ar[d,e,f]*isoR'[g,f,e]
          # H0 = SparseProjectiveHamiltonian(El, Er, (), [H.si[2],H.si[1]]) 
          # ~, xg, info = eigsolve(x -> H0 * x, x0, 1, :SR, _getLanczos(; kwargs...))

          # # reshape back
          # @tensor x0[a b;f e] := isoL[a,b,c]*xg[1][c,d]*isoR[e,f,d]
          # return x0, info
     else
          x2 = CompositeMPSTensor(Al, Ar)
          @timeit TO "LanczosGS" eg, xg, info = eigsolve(x -> action2(H, x; kwargs...), x2, 1, :SR, _getLanczos(; kwargs...))
