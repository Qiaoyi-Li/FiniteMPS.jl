function _CBE_MMd2US(lsEr::SparseRightTensor;
     ...
     # M = USV' => MM' = U S^2 U', return U*S, ϵ
     # for sparse case, contract c index and manually sum over b index
     #   a--
     #      |
     #   b--Er  --> a--Er*Er'--a'
     #      |
...
function _CBE_MdM2US(lsEl::SparseLeftTensor;
     trunc::TruncationScheme=TensorKit.NoTruncation(),
     normalize::Bool=false)
     # M = USV' => M'M = V S^2 V', return U*S = M*V, ϵ
     # note M*V is a sparce environment tensor
     # for sparse case, contract a index and manually sum over b index
     #    --c
     #   |
     #  El--b --> c'--El'*El--c
     #   |
