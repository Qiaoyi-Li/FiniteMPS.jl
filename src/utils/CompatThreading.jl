...
#             TensorKit.TensorOperations.tensoradd!(tdst[f₁′, f₂′], p, tsrc[f₁, f₂], :N, α * coeff, true,
#                           backend...)
#         end
#     end
...
function TensorKit._add_abelian_kernel!(tdst, tsrc, p, fusiontreetransform, α, β, backend...)
    ...
    #         Threads.@spawn TensorKit._add_abelian_block!(tdst, tsrc, p, fusiontreetransform,
    #             f₁, f₂, α, β, backend...)
    #     end
    # else
...
function mul!(tC::AbstractTensorMap,
    ...
    NWORKER = get_num_workers()
    NTHREADS = get_num_threads_julia()

    # parellel this function only in multi-processing mode
    if NWORKER > 1 && NTHREADS > 1

        @floop TaskPoolEx(; ntasks=NTHREADS - 1, simd=true) for c in blocksectors(tC)
