...
# 		fermionic[1] && fermionic[2] && (strength *= -1)
# 	end
# 	if si[2] > si[3]
# 		B, C = _swap(B, C)
# 		si = (si[1], si[3], si[2])
# 		fermionic = (fermionic[1], fermionic[3], fermionic[2])
# 		fermionic[2] && fermionic[3] && (strength *= -1)
# 	end
# 	if si[1] > si[2]
# 		A, B = _swap(A, B)
# 		si = (si[2], si[1], si[3])
...
# 	if si[1] == si[2] == si[3]
# 		return addIntr1!(Root, A * B * C, strength; value = value)
...
# 	isnothing(Z) && @assert !any(fermionic)

# 	# check rank 
# 	@assert rank(A, 1) == rank(C, 2) == 1
# 	ranks = (rank(A, 2), rank(B, 1), rank(B, 2), rank(C, 1))
# 	@assert ranks in [(1, 1, 1, 1), (2, 2, 1, 1), (2, 1, 1, 2), (1, 1, 2, 2), (2, 2, 2, 2)]
...
# _addtag!(::LocalOperator{1, 2}, ::LocalOperator{1, 1}, ::LocalOperator{2, 1}) = nothing
# _addtag!(::LocalOperator{1, 1}, ::LocalOperator{1, 2}, ::LocalOperator{2, 1}) = nothing
# _addtag!(::LocalOperator{1, 2}, ::LocalOperator{2, 1}, ::LocalOperator{1, 1}) = nothing
